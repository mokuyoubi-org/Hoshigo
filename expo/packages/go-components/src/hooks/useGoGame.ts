...
export function useGoGame({ boardSize, matchType, movesInt }: Args) {
  ...
  const agehamaHistoryRef = useRef<AgehamaCount[]>(initialState.agehamaHistory);
  const [agehamaHistory, setAgehamaHistoryState] = useState<AgehamaCount[]>(
  ...
  const pushAgehama = (agehama: AgehamaCount) => {
