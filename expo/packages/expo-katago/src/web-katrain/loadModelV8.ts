import { KataGoBinModelParser, parseConv2d, parseMatMul, parseBatchNormV8, parseActivationKind, parseMatBias } from "./binModelParser";
import { ParsedKataGoModelV8 } from "./modelV8";
...
export function parseKataGoModelV8(data: Uint8Array): ParsedKataGoModelV8 {
  ...
  if (modelVersion < 8 || modelVersion > 14) {
    throw new Error(
      `Unsupported modelVersion ${modelVersion}, supported 8..14`,
    );
  }
  ...
  function parseResidualBlock(): ParsedKataGoModelV8["trunk"]["blocks"][number] {
    ...
    if (kindTok === "nested_bottleneck_block") {
      ...
      for (let i = 0; i < numInnerBlocks; i++)
        blocks.push(parseResidualBlock());
