...
import { candidateGridsForMove, computeMoveEvaluations } from "@/src/stable/logics/moveEvaluation";
...
export function useReplayMoveEvaluation(
  ...
  const moveEvaluations = useMemo(
    () => computeMoveEvaluations(record.analysis, record.moves, boardSize, record.match_type),
    [record.analysis, record.moves, boardSize, record.match_type],
  );

  const lastMoveIndex = currentIndex - 1;
  const lastMoveEvaluation =
    !isEditMode && lastMoveIndex >= 0 ? moveEvaluations[lastMoveIndex] : undefined;
  const lastMoveGrid =
    !isEditMode && lastMoveIndex >= 0 ? (record.moves?.[lastMoveIndex] ?? null) : null;
