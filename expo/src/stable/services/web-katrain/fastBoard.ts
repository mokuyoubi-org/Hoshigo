...
import { BLACK, Color, EMPTY, PASS_GRID, WHITE } from "../../types/goTypes";

/*
🌟export let BOARD_SIZE = 9;についての説明🌟
このファイルのすぐ下の方に

export const initBoardArrays = (size: number): void => {
...
  BOARD_SIZE = size;
  ...}
とあり、
...
...
  ...
  ...}
  とあり、katagoを使用する際にはちゃんと適切な盤面サイズが渡されるようになっているので、ここが19固定のように見えてもそれは初期値に過ぎないので問題はない。
*/
...
export function getBoardArea(): number {
  return BOARD_AREA;
}
export function getPassMove(): number {
  return PASS_GRID;
}
...
export const initBoardArrays = (size: number): void => {
  console.log("initBoardArrays called. Board size: ", size);
  ...
  console.log("BOARD_AREA: ", BOARD_AREA);
  ...
  // initBoardArrays の一番最後に
  console.log(
    "initBoardArrays complete. BOARD_AREA: ",
    BOARD_AREA,
    "BOARD_SIZE: ",
    BOARD_SIZE,
  );
  console.log("Module identity check: ", Math.random()); // ← これが重要
};
...
export function playMove(
  ...
  const opp = getOppositeColor(player);
  ...
  for (let i = 0; i < mCount; i++) {
    ...
    if ((pos.stones[n] as Color) !== opp) continue;
    if (PROCESSED_GROUP[n] === pStamp) continue;

    const { groupLen, liberties } = collectGroupAndLiberties(
      pos.stones,
      n,
      opp,
      1,
    ...
    if (totalCaptured === 1 && groupLen === 1)
      capturedSinglePos = GROUP_BUF[0]!;
...
export function computeLibertyMapInto(
  stones: Uint8Array,
  out: Uint8Array,
): Uint8Array {
  if (out.length !== BOARD_AREA)
    throw new Error(
      `computeLibertyMapInto: expected out length ${BOARD_AREA}, got ${out.length}`,
    );
  out.fill(0);
  groupSeenStamp++;
...
export function updateLibertyMapForSeeds(
  ...
  if (out.length !== BOARD_AREA)
    throw new Error(
      `updateLibertyMapForSeeds: expected out length ${BOARD_AREA}, got ${out.length}`,
    );
...
function buildGroups(stones: Uint8Array): number {
  ...
  for (let p = 0; p < BOARD_AREA; p++) {
    ...
    while (stackLen > 0) {
      ...
      for (let i = 0; i < nCount; i++) {
        const n = NEIGHBORS[nStart + i]!;
        if ((stones[n] as Color) !== c) continue;
...
function isAdjacentToColor(
  ...
  for (let i = 0; i < nCount; i++) {
    const n = NEIGHBORS[nStart + i]!;
    if ((stones[n] as Color) === color) return true;
  }
  return false;
}
...
function isAdjacentToPlaGroup(
  ...
  for (let i = 0; i < nCount; i++) {
    const n = NEIGHBORS[nStart + i]!;
    if ((stones[n] as Color) !== plaColor) continue;
...
function calculateAreaForPla(args: {
  ...
  const oppColor = getOppositeColor(plaColor);
  ...
  const buildRegion = (initialPos: number, regionIdx: number): number => {
    ...
    while (qh !== qt) {
      ...
      if (
        hasVital &&
        (isMultiStoneSuicideLegal || (stones[pos] as Color) === EMPTY)
      ) {
      ...
      if (
        NUM_INTERNAL_SPACES_MAX2[regionIdx]! < 2 &&
        !isAdjacentToColor(stones, pos, plaColor)
      ) {
        NUM_INTERNAL_SPACES_MAX2[regionIdx] = (NUM_INTERNAL_SPACES_MAX2[
          regionIdx
        ]! + 1) as number;
      }

      if ((stones[pos] as Color) === oppColor) CONTAINS_OPP[regionIdx] = 1;
  ...
  for (let p = 0; p < BOARD_AREA; p++) {
    ...
    let initialVLen = 0;
    {
      ...
      for (let i = 0; i < nCount; i++) {
        ...
        if ((stones[adj] as Color) !== plaColor) continue;
  ...
  for (let g = 0; g < numGroups; g++) {
    if ((GROUP_COLOR_BY_GROUP[g] as Color) === plaColor) {
      ...
      for (let t = 0; t < len; t++) {
        ...
        for (let k = 0; k < nCount; k++) {
          ...
          for (let u = 0; u < vl; u++) {
            ...
            VITAL_COUNT_BY_GROUP[gg] = (VITAL_COUNT_BY_GROUP[gg]! -
              1) as number;
          }
        }
      }
  ...
  for (let i = 0; i < numRegions; i++) {
    ...
    let shouldMark =
      NUM_INTERNAL_SPACES_MAX2[i]! <= 1 &&
      !BORDERS_NONPASSALIVE_BY_HEADPOS[headPos] &&
      atLeastOnePla;
    shouldMark =
      shouldMark ||
      (safeBigTerritories &&
    ...
    if (shouldMark) {
      ...
      const shouldMarkIfEmpty =
        unsafeBigTerritories && !CONTAINS_OPP[i] && atLeastOnePla;
      if (shouldMarkIfEmpty) {
        let cur = headPos;
        do {
          if ((result[cur] as Color) === EMPTY) result[cur] = plaColor;
          cur = NEXT_EMPTY_OR_OPP[cur]!;
        } while (cur !== headPos);
      }
    }
...
export function computeAreaMapV7KataGo(
  stones: Uint8Array,
  isMultiStoneSuicideLegal = false,
): Uint8Array {
  return computeAreaMapV7KataGoInto(
    stones,
...
export function computeAreaMapV7KataGoInto(
  ...
  if (out.length !== BOARD_AREA)
    throw new Error(
      `computeAreaMapV7KataGoInto: expected out length ${BOARD_AREA}, got ${out.length}`,
    );
  out.fill(EMPTY);
  ...
  for (let p = 0; p < BOARD_AREA; p++) {
    if ((out[p] as Color) === EMPTY) out[p] = stones[p]!;
  }
...
function isAdjacent(a: number, b: number): boolean {
  ...
  const d = a - b;
  if (d === 1 || d === -1)
    return ((a / BOARD_SIZE) | 0) === ((b / BOARD_SIZE) | 0);
...
function getNumImmediateLiberties(stones: Uint8Array, pos: number): number {
  ...
  for (let i = 0; i < nCount; i++) {
    const n = NEIGHBORS[nStart + i]!;
    if ((stones[n] as Color) === EMPTY) num++;
  }
  return num;
}

function findLibertiesIntoBuf(
  stones: Uint8Array,
  start: number,
  color: Color,
  buf: Int16Array,
...
function getNumLibertiesCapped(
  stones: Uint8Array,
  loc: number,
  cap: number,
): number {
  const c = stones[loc] as Color;
  if (c === EMPTY) return 0;
...
function wouldBeKoCapture(
  ...
  if ((stones[loc] as Color) !== EMPTY) return false;
...
function hasLibertyGainingCaptures(stones: Uint8Array, loc: number): boolean {
  ...
  for (let i = 0; i < g.groupLen; i++) {
    ...
    for (let j = 0; j < nCount; j++) {
      ...
      if ((stones[adj] as Color) !== opp) continue;
...
function countHeuristicConnectionLibertiesX2(
  stones: Uint8Array,
  loc: number,
  pla: Color,
): number {
  ...
  for (let i = 0; i < nCount; i++) {
    const adj = NEIGHBORS[nStart + i]!;
    if ((stones[adj] as Color) !== pla) continue;
...
function getBoundNumLibertiesAfterPlay(
  ...
  const lowerBound =
    numCaps +
    (maxConnectionLibs > numImmediateLibs
      ? maxConnectionLibs
      : numImmediateLibs);
  const upperBound =
    numImmediateLibs + potentialLibsFromCaps + numConnectionLibs;
  return { lowerBound, upperBound };
}

function getNumLibertiesAfterPlay(
  stones: Uint8Array,
  loc: number,
  pla: Color,
  max: number,
): number {
  if ((stones[loc] as Color) !== EMPTY) return 0;
  const opp = getOppositeColor(pla);
  ...
  for (let i = 0; i < nCount; i++) {
    ...
    if (c === EMPTY) {
      ...
      if (oppInfo.liberties === 1) {
        ...
        addLib(adj);
        if (numLibs >= max) return max;
        for (let j = 0; j < oppInfo.groupLen; j++)
          LADDER_CAPTURED[GROUP_BUF[j]!] = capStamp;
      }
    }
  }
  ...
  for (let i = 0; i < nCount; i++) {
    ...
    for (let j = 0; j < g.groupLen; j++)
      LADDER_CONNECT_GROUP_SEEN[GROUP_BUF[j]!] = seenStamp;
...
function findLibertyGainingCaptures(
  ...
  for (let i = 0; i < g.groupLen; i++) {
    ...
    for (let j = 0; j < nCount; j++) {
      ...
      const libCount = findLibertiesIntoBuf(
        stones,
        adj,
        opp,
        buf,
      ...
      for (let k = 0; k < groupLen; k++)
        LADDER_OPP_GROUP_SEEN[GROUP_BUF[k]!] = seenStamp;
...
function undoMoveRaw(
  ...
  if (move !== PASS_GRID) pos.stones[move] = EMPTY;
...
function tryPlayMoveNoThrow(
  ...
  if ((pos.stones[move] as Color) !== EMPTY) return false;
...
function searchIsLadderCaptured(
  ...
  const c = pos.stones[loc] as Color;
  ...
  while (true) {
    ...
    if (searchNodeCount >= LADDER_SEARCH_NODE_BUDGET) {
      for (let i = stackIdx - 1; i >= 0; i--) {
        undoMoveRaw(
          pos,
          recordMoves[i]!,
          recordPlayers[i]! as Color,
          recordKoPointBefore[i]!,
          recordCaptureStart[i]!,
          captureStack,
        );
      }
      pos.koPoint = koSaved;
    ...
    if (moveListCur[stackIdx] === -1) {
      ...
      if (isDefender) {
        moveListLen = findLibertyGainingCaptures(
          pos.stones,
          loc,
          bufMoves,
          start,
        );
        moveListLen += findLibertiesIntoBuf(
          pos.stones,
          loc,
          pla,
          bufMoves,
        ...
        if (
          libsA === 0 &&
          libsB === 0 &&
          wouldBeKoCapture(pos.stones, bufMoves[start]!, opp) &&
          wouldBeKoCapture(pos.stones, bufMoves[start + 1]!, opp)
        ) {
          if (
            getNumLibertiesAfterPlay(pos.stones, bufMoves[start]!, pla, 3) <=
              2 &&
            getNumLibertiesAfterPlay(
              pos.stones,
              bufMoves[start + 1]!,
              pla,
              3,
        ...
        if (moveListLen > 1) {
          libsA =
            libsA * 2 +
            countHeuristicConnectionLibertiesX2(
              pos.stones,
              bufMoves[start]!,
              pla,
            );
      ...
      if (returnedFromDeeper) {
        undoMoveRaw(
          ...
          recordPlayers[stackIdx]! as Color,
    ...
    moveListStarts[stackIdx] =
      moveListStarts[stackIdx - 1]! + moveListLens[stackIdx - 1]!;
...
function searchIsLadderCapturedAttackerFirst2Libs(
  ...
  if (
    tryPlayMoveNoThrow(
      ...
      move0,
      opp,
      scratch.captureStack,
      scratch.tmpKoPointBefore,
      scratch.tmpCaptureStart,
      0,
    )
  ) {
    ...
    undoMoveRaw(
      ...
      move0,
      opp,
      scratch.tmpKoPointBefore[0]!,
      scratch.tmpCaptureStart[0]!,
      scratch.captureStack,
    );
  }
...
export function computeLadderFeaturesV7KataGoInto(args: {
  ...
  const {
    ...
    currentPlayer,
    outLadderedStones,
    outLadderWorkingMoves,
  } = args;
  outLadderedStones.fill(0);
  outLadderWorkingMoves.fill(0);

  const opp = getOppositeColor(currentPlayer);
  ...
  for (let p = 0; p < BOARD_AREA; p++) {
    ...
    for (let i = 0; i < g.groupLen; i++)
      LADDER_GROUP_SEEN[GROUP_BUF[i]!] = seenStamp;
    ...
    if (g.liberties === 1) {
      ...
      laddered = searchIsLadderCapturedAttackerFirst2Libs(
        copyPos,
        p,
        LADDER_SCRATCH,
        workingMoves,
    ...
    if (g.liberties === 2 && c === opp && workingMoves.length > 0) {
      for (let i = 0; i < workingMoves.length; i++)
        outLadderWorkingMoves[workingMoves[i]!] = 1;
    }
  }
}
...
export function computeLadderedStonesV7KataGoInto(args: {
  ...
  for (let p = 0; p < BOARD_AREA; p++) {
    ...
    if (g.liberties === 1)
      ladderedGroup = searchIsLadderCaptured(copyPos, p, true, LADDER_SCRATCH);
    else
      ladderedGroup = searchIsLadderCapturedAttackerFirst2Libs(
        copyPos,
        p,
        LADDER_SCRATCH,
        workingMoves,
...
export function computeLadderedStonesV7KataGo(args: {
  stones: Uint8Array;
  koPoint: number;
}): Uint8Array {
