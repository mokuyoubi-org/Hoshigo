import type {
  ...
  ParsedMatBias,
  ParsedMatMul,
} from "@/src/stable/services/web-katrain/binModelParser";
...
//   const arr = Array.from(conv.weights); // Float32Array → 普通のArray に変換
//   const filter = tf.tensor4d(arr, [
//     conv.kernelY,
//     conv.kernelX,
//     conv.inChannels,
//     conv.outChannels,
...
// function makeConv(conv: ParsedConv2d): TfConv {
//   // 🟩 普通のArrayに変換せず、Float32Arrayのまま扱うにゃ！
//   const weights =
//     conv.weights instanceof Float32Array
//       ? conv.weights
//       : new Float32Array(conv.weights);

//   const filter = tf.tensor4d(weights, [
//     conv.kernelY,
//     conv.kernelX,
//     conv.inChannels,
//     conv.outChannels,
...
function makeConv(conv: ParsedConv2d): TfConv {
  ...
  // 🟩 tensor4d の代わりに tf.tensor を使い、4Dテンソルとして作成するにゃ！
  const filter = tf.tensor(
    weights,
    [conv.kernelY, conv.kernelX, conv.inChannels, conv.outChannels],
    "float32",
  ) as tf.Tensor4D;
...
function poolRowsGPool(x: tf.Tensor4D): tf.Tensor2D {
  ...
  const boardSize = x.shape[1] ?? 19; // 19固定。nnに渡すtensorのshapeなので
...
function poolRowsValueHead(x: tf.Tensor4D): tf.Tensor2D {
  ...
  const mean = tf.mean(x, [1, 2]) as tf.Tensor2D; // [N,C]
  return tf.concat(
    [mean, mean.mul(factor1), mean.mul(factor2)],
    1,
  ) as tf.Tensor2D;
}
...
export class KataGoModelV8Tf {
  ...
  forward(
    ...
    return tf.tidy(() => {
      ...
      p1Out = p1Out.add(
        g1Bias.reshape([g1Bias.shape[0], 1, 1, g1Bias.shape[1]]),
      ) as tf.Tensor4D;
  ...
  private forwardTrunk(spatial: tf.Tensor4D, global: tf.Tensor2D): tf.Tensor4D {
    ...
    trunk = trunk.add(
      ginput.reshape([ginput.shape[0], 1, 1, ginput.shape[1]]),
    ) as tf.Tensor4D;
  ...
  private applyBlockStack(
    ...
    for (const block of blocks) {
      ...
      if (block.kind === "gpool") {
        ...
        regularOut = regularOut.add(
          gpoolBias.reshape([gpoolBias.shape[0], 1, 1, gpoolBias.shape[1]]),
        ) as tf.Tensor4D;
