import { BOARD_SIZE } from "@/src/stable/services/web-katrain/fastBoard";
import { getOpponent } from "@/src/stable/services/web-katrain/gameLogic";
import { BoardState, GameRules } from "@/src/stable/services/web-katrain/types";
import { BLACK, Color, KataGoMove, WHITE } from "@/src/stable/types/goTypes";
...
export function fillInputsV7(args: {
  ...
  for (let pos = 0; pos < stones.length; pos++) {
    ...
    const plane =
      liberties === 1 ? 3 : liberties === 2 ? 4 : liberties === 3 ? 5 : -1;
    if (plane >= 0) {
  ...
  const passWouldEndGame =
    !!lastMove && (lastMove.x === -1 || lastMove.y === -1);
