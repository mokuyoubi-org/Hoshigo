...
import { getOpponent } from "@/src/stable/services/web-katrain/gameLogic";
import { GameRules } from "@/src/stable/services/web-katrain/types";
...
const idxNHWC = (x: number, y: number, c: number) =>
  (y * BOARD_SIZE + x) * INPUT_SPATIAL_CHANNELS_V7 + c;

function playerToColor(p: Color): Color {
...
export function fillInputsV7Fast(args: {
  ...
  areaMap?: Uint8Array; // KataGo-style area map for planes 18
  ...
  for (let pos = 0; pos < BOARD_SIZE * BOARD_SIZE; pos++)
    spatial[pos * INPUT_SPATIAL_CHANNELS_V7 + 0] = 1.0;
  ...
  if (
    args.ladderedStones ||
    args.prevLadderedStones ||
    args.prevPrevLadderedStones ||
    args.ladderWorkingMoves
  ) {
  ...
  if (rules === "chinese") {
    ...
    for (let y = 0; y < BOARD_SIZE; y++) {
      for (let x = 0; x < BOARD_SIZE; x++) {
        const pos = y * BOARD_SIZE + x;
        const v = area[pos] as Color;
        if (v === plaColor) spatial[idxNHWC(x, y, 18)] = 1.0;
        else if (v === oppColor) spatial[idxNHWC(x, y, 19)] = 1.0; // 無関係の19。features数
      }
    }
  }
  ...
  const passWouldEndGame = lastMove?.move === PASS_GRID;
  const suppressHistory =
    args.conservativePassAndIsRoot === true && passWouldEndGame;
...
export function extractInputsV7Fast(args: {
  ...
  const spatial = new Float32Array(
    BOARD_SIZE * BOARD_SIZE * INPUT_SPATIAL_CHANNELS_V7,
  );
