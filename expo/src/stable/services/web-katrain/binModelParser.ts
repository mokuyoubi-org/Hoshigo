export class KataGoBinModelParser {
  ...
  readBinaryFloats(count: number): Float32Array {
    ...
    if (absEnd > this.data.buffer.byteLength)
      throw new Error("Unexpected EOF while reading binary floats");
    ...
    // React Native では ArrayBuffer.slice が動かないため、コピーで対処
    const result = new Float32Array(count);
    const view = new DataView(this.data.buffer, absStart, byteLen);
...
export function parseBatchNormV8(p: KataGoBinModelParser): ParsedBatchNorm {
  ...
  const scale = hasScale
    ? p.readBinaryFloats(channels)
    : new Float32Array(channels).fill(1);
  const bias = hasBias
    ? p.readBinaryFloats(channels)
    : new Float32Array(channels).fill(0);
...
export function parseActivationKind(
  p: KataGoBinModelParser,
  modelVersion: number,
): ActivationKind {
...
export function parseConv2d(p: KataGoBinModelParser): ParsedConv2d {
  ...
  const weights = p.readBinaryFloats(
    kernelY * kernelX * inChannels * outChannels,
  );
  return {
    name,
    kernelY,
