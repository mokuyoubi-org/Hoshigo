// 移植
import { BoardState } from "@/src/stable/services/web-katrain/types";
import { BLACK, Color, WHITE } from "../../types/goTypes";
...
export const getLiberties = (
  ...
  x: number,
  y: number,
): { liberties: number; group: { x: number; y: number }[] } => {
...
export const applyCapturesInPlace = (
  board: BoardState,
  x: number,
  y: number,
  player: Color,
...
export const checkCaptures = (
  ...
  y: number,
  player: Color,
): { captured: { x: number; y: number }[]; newBoard: BoardState } => {
