...
export  const printCustomKataGoResult=(board:any, moves: any, currentPlayer: any, result: any): void =>{
...
printDebugBoard(board)
  ...
  console.log("currentPlayer: ", currentPlayer)
    ...
    console.log("出力情報:")
      // 小数を第2位で切り捨てるおまじないだにゃ
      const trunc = (val: number): number => Math.floor(val * 100) / 100;

      // 1. moves を 0~4 まで取得して、1行ずつのJSON文字列にするにゃ！
      ...
      // 2. ownership の配列内の小数をまとめて切り捨てるにゃ（1行にまとめる！）
      const ownershipFormatted = JSON.stringify(result.ownership.map(trunc));

      // 3. 社長が作ってくれた理想のフォーマットに組み立てるにゃ！
      const output =
        `{"moves": [\n` +
        `${movesFormatted}], \n` +
