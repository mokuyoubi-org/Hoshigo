...
export function parseKorean(text: string): string {
  let retval = "";
  let lastCharIsHangul = false;
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (charCode >= kromanData.ga && charCode <= kromanData.hih) {
      ...
      retval +=
        kromanData.headj[head as keyof typeof kromanData.headj] +
        kromanData.bodyj[body as keyof typeof kromanData.bodyj] +
        kromanData.tailj[tail as keyof typeof kromanData.tailj];
